...
| **天气/位置**       | 浏览器定位 → 城市名（OSM Nominatim），按日期保存 |
...
- **格子操作**：
  ...
  - 点击 ✕ → 画红色叉（已取消）
  - 点击 ↻ → 恢复原状
  - 所有状态下都可填写说明：完成/取消说明为红色，普通进展说明为蓝色
...
- **今天**始终显示天气/位置行，便于手动点击 📍 获取
- **其他日期**只在当天已有记录时显示（避免未来日期显示假数据）
- 提取纯描述（去温度）：`晴 23°C` → `晴`
- 仅显示省市级：`北京朝阳`、`山西运城`
...
从 v2.1 起，写入前会用 `CompressionStream('gzip')` 压缩，存储格式为 `GZ|` 前缀 + base64；不支持压缩的浏览器自动回退为纯 JSON，旧数据也能被自动识别读取。
...
```js
{
  cells: {
    "2026-08-27": {
      pages: [
        {
          1: { content: "...", status: "active|completed|cancelled", note: "..." },
          2: { content: "...", status: "..." }
        }
      ]
    }
  ...
  weatherByDate: {
    "2026-08-31": { location, weather, city, source, hasGeo }
  },
  settings: {
...
**推荐方式**：使用侧边栏「导出 · 导入 · 查看」三个按钮，无需手动操作控制台。
...
```js
localStorage.setItem('one-page-planner:v1', /* 粘贴导出的内容 */)
location.reload()
```

### 容量与归档

- localStorage 单 origin 通常约 5 MB（Chrome / Firefox / Safari）。
- 数据写入前会 gzip 压缩，Markdown 文本通常可压到约 30% 左右。
- 「查看」面板会显示已用容量、压缩率与预计可用年限。
- 容量超过 80% 时侧边栏会提示「存储将满」。
- 「查看」面板底部可「归档 1 年前数据」，归档数据导出下载后从热数据移除。
...
### v2.1 (2026-08-28) — 月视图布局、格子说明、导入导出、存储优化

#### 📅 任务四：月视图重要日子与事项预览分离

- 月视图单元格顶部行合并日期号与进度条，节省垂直空间
- 重要日子用金色圆点标签条放在顶部，与事项预览视觉分隔不再遮挡
- 修复「某天无事项但有重要日子」时漏显的问题

#### 📝 任务五：完成/取消格子可追加说明

- `cell` 数据结构扩展为 `{ content, status, note }`
- 已完成/取消的格子点击仍可编辑，可补充「完成说明」或「取消原因」
- 说明文字以红色字显示（`.status-note`，沿用 `--accent`）
- 恢复原状时自动清空说明；旧数据无 `note` 字段自动兼容

#### 📤 任务六：侧边栏导入 / 导出 / 查看

- 侧边栏顶部新增「导出 · 导入 · 查看」三个按钮
- 导出：下载 `one-page-planner-YYYYMMDD.json`
- 导入：选择 JSON 文件，先弹确认（含数据预览：X 天数据 / Y 个重要日子），确认后覆盖并自动迁移
- 查看：只读弹窗展示日期数、重要日子数、JSON 大小、容量占用

#### 💾 任务七：存储容量评估与优化

- 存储压缩：写入时用 `CompressionStream('gzip')` 压缩为 `GZ|` 前缀 + base64，读取时解压；不支持时回退纯 JSON
- 自动归档：查看面板新增「归档 1 年前数据」按钮，归档数据导出下载后从热数据移除
- 容量监控：启动时超过 80% 阈值时侧边栏提示「存储将满」
- 容量不足降级：捕获 `QuotaExceededError` 并提示归档/导出
- 查看面板显示压缩率、已用容量、预计可用年限

### 导入 / 导出 / 归档 用法

1. **导出备份**：侧边栏点「导出」，浏览器会下载当前全部数据为 JSON 文件。
2. **导入恢复**：侧边栏点「导入」，选择之前导出的 JSON 文件；确认弹窗会显示将导入的数据量，确认后覆盖当前数据。
3. **查看核对**：侧边栏点「查看」，弹窗展示数据统计与完整 JSON（只读）。
4. **归档旧数据**：在「查看」弹窗底部点「归档 1 年前数据」，把一年前的数据导出下载并从当前数据移除，释放存储空间。

### v2.2 (2026-08-28) — 启动行为与定位体验

- **启动回到今天**：每次打开或刷新页面，固定回到「今天的日视图」，不会停留在上次的月/周/年视图或其他日期。（修复：页面加载时自测代码会把视图临时切到月视图，导致刷新后停留在月界面；现在启动流程会同步切换视图高亮与显示区域。）
- **定位一天问一次**：定位结果与当天的天气、位置会保存到本地；同一天内再次打开不再重复请求定位授权，直接复用已保存的天气和位置。
- **拒绝定位当天不再打扰**：如果当天拒绝过定位，当天内不再反复弹出授权请求。
- 修复完成/取消格子说明框的编辑交互：点击红色说明框不再被误销毁，焦点在正文与说明框之间切换时不触发保存。

### v2.3 (2026-08-31) — 天气按日保存 + 普通进展说明

- **天气/位置按日期保存**：每次获取成功会写入 `weatherByDate[当天]`，回到过去日期仍能看到当天记录的天气与位置；次日获取失败时会回退到最近一次已知位置，不再显示空白。
- **普通进展说明**：所有格子状态都可填写说明。未标记完成/取消的格子说明显示为蓝色，完成/取消说明仍为红色。

### 部署地址

仓库：<https://github.com/zzx33522/one-page-planner>
...
| 2.1 | 2026-08-28 | 月视图布局、格子说明、导入导出、存储压缩归档       |
| 2.2 | 2026-08-28 | 启动回今天、定位一天问一次、说明框编辑修复         |
